...
class TensorBackend:
    def __init__(self, bitwidth=8, format="float", tile_latency=12, persistent_core=True):
        ...
        # Reuse the same hardware object across calls
    ...
    def matmul_fn(self, A, B, C):
        ...
        if A.shape[1] != B.shape[0]:
            raise ValueError(
                f"Incompatible shapes for matmul: A{A.shape} and B{B.shape}"
            )

        expected_c_shape = (A.shape[0], B.shape[1])
        if C.shape != expected_c_shape:
            raise ValueError(
                f"C must have shape {expected_c_shape}, but got {C.shape}"
            )
        ...
        # If quantized hardware is added later.
        if self.format == "int":
            return A @ B + C
