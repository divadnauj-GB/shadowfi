...
def create_detailed_report(sig_list, o_filename, fault_type):
    ...
    get_workdir = o_filename.replace(o_filename.split("/")[-1],"")
    outcsv_file = get_workdir+"/"+"report_detailed_" + fault_type + ".csv"
    # outcsv_file = "report_detailed_" + ".csv"
        ...
        for i, x in enumerate(sig_list):        
            if x.get_numBit() == 1:             
                ...
                for k in range(x.get_numBit()):
                    writer.writerow([sr_no, f"{x.get_name()}[{k}]", x.get_type(), x.get_lineNum(), o_filename, path_csv])
                    sr_no += 1

        #if("SABOTUER" in fault_type ):    # add control bits
