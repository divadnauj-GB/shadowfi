...
FPGA_cluster = init_fpga(configname='TCU_2_SBTR',nodes_selected=[2])
...
def finj_task(input_data,f_list=[],fi_config={}):
    ...
    def tcu_compute(data_input):
        ...
        try:
            ...
            for MM in data_input:    
                (data, elements, bit_over_flow, bit_almost_full, bit_full)=cb0.fifo_out_status()
                while(bit_almost_full):
                    (data, elements, bit_over_flow, bit_almost_full, bit_full)=cb0.fifo_out_status()                
                cb0.write_fifo(MM[:48])
                get_data=[]
                cnt=0
                while(cnt<1000):
                    rd_fl = cb0.fifo_in_elements()
                    if rd_fl!=0:
                        get_data.extend(cb0.read_fifo(rd_fl))
                    if(len(get_data)==16):
                        break
                    cnt=cnt+1 
                
                Read_fifo.append(get_data)                
        finally:
            print("got here")
        return Read_fifo
